...
2. **注册流程**:
   - 首次访问时,控制台显示**注册页面**
   - 创建管理员账户(用户名 + 密码)
   - 管理员可以管理所有智能体,并为每个智能体创建可选的专属登录账户
3. **登录流程**:
   - 注册完成后,后续访问显示**登录页面**
   - 输入凭据后,生成签名令牌(有效期 7 天)
   - 令牌存储在浏览器 localStorage,自动附加到所有 API 请求
   - 智能体专属账户登录后会自动进入绑定的智能体,且不能操作其他智能体
4. **自动注册**(可选):
...
- 管理员和智能体专属账户密码均以加盐 SHA-256 哈希存储,不存储明文
- HMAC-SHA256 签名令牌,7 天自动过期
- 智能体专属账户令牌携带绑定的 `agent_id`,访问其他智能体时会被拒绝
- 仅使用 Python 标准库(`hashlib`、`hmac`、`secrets`),无外部依赖
- `auth.json` 文件以 `0o600` 权限保护(仅所有者可读写)

### 智能体专属账户

管理员可以为每个智能体创建独立的 Web 登录账户。智能体账户登录后,控制台会自动选中该智能体,并且只开放该智能体相关的操作。
...
2. 打开 **设置 -> 智能体**
3. 创建或编辑一个智能体
4. 填写 **Web 登录用户名** 和 **Web 登录密码**
5. 保存智能体后,即可在登录页使用该账号密码登录

| 账户类型       | 作用范围                   | 典型操作                           |
| -------------- | -------------------------- | ---------------------------------- |
| 管理员账户     | 所有智能体和全局控制台设置 | 创建、删除、排序、启用和编辑智能体 |
| 智能体专属账户 | 仅绑定的智能体             | 使用聊天/任务并更新该智能体设置    |

智能体密码在控制台中只写不可读:保存后的哈希不会再次显示。修改密码会替换已保存的哈希,后续登录需要使用新密码。智能体账户在智能体设置界面管理,不通过环境变量配置。
...
- 这些变量只用于创建管理员账户;智能体专属账户在 **设置 → 智能体** 中配置
- 如果已有管理员账户,自动注册环境变量会被忽略
